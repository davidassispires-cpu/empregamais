...
function candidatoPremiumAtivoEM(c){c=c||candidatoLogado();return !!(c&&c.premium===true)}
...
function distanciaVagaTextoEM(v){
 if(!localizacaoCandidatoEM||String(v?.modalidade||'').toLowerCase().includes('remot'))return'';
 const km=distanciaKmEM(localizacaoCandidatoEM.latitude,localizacaoCandidatoEM.longitude,v?.latitude,v?.longitude);
