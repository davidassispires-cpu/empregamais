...
function cardVagaPortal(v){
 ...
 return '<article class="vaga-card portal-vaga portal-vaga-nova '+(logo?'com-logo ':'sem-logo ')+(destaqueAtivo(v)?'destaque ':'')+(v.urgente?'urgente':'')+'" onclick="abrirVaga(\''+v.id+'\')"><div class="vaga-card-corpo"><div class="vaga-identidade">'+logoHtml+'<div class="vaga-conteudo"><div class="vaga-empresa-linha"><span class="vaga-empresa-nome">'+esc(nome)+seloEmpresa+'</span><div class="vaga-selos vaga-selos-empresa">'+tags+'</div></div><h3>'+esc(tituloVaga(v))+'</h3><div class="vaga-meta"><span><i class="meta-ico">⌖</i>'+esc(v.cidade||'')+(v.estado?' - '+esc(v.estado):'')+(distanciaVagaTextoEM(v)?'<small class="vaga-distancia-em"> · '+esc(distanciaVagaTextoEM(v))+'</small>':'')+'</span><span><i class="meta-ico">▣</i>'+esc(v.modalidade||'')+'</span><span><i class="meta-ico">▤</i>'+esc(v.contrato||'')+'</span>'+salarioMeta+'</div><p class="vaga-resumo">'+esc(desc?desc.slice(0,240):'Confira os detalhes completos desta oportunidade.')+(desc.length>240?'…':'')+'</p></div></div></div><aside class="vaga-card-acao"><button type="button" class="vaga-ver-btn" onclick="event.stopPropagation();abrirVaga(\''+v.id+'\')">Ver vaga</button>'+dataPublicacao+'</aside></article>';
}
