...
function cardVagaPortal(v){
 ...
 return '<article class="vaga-card portal-vaga portal-vaga-nova '+(destaqueAtivo(v)?'destaque ':'')+(v.urgente?'urgente':'')+'" onclick="abrirVaga(\''+v.id+'\')"><div class="vaga-card-topo"><div class="vaga-selos">'+tags+'</div><button type="button" class="vaga-favorito" aria-label="Salvar vaga" onclick="event.stopPropagation();alternarSalvarVagaCard(\''+v.id+'\')">'+(vagaEstaSalva(v.id)?'♥':'♡')+'</button></div><div class="vaga-identidade">'+(logo?'<img class="vaga-logo" src="'+esc(logo)+'" alt="Logo '+esc(nome)+'" loading="lazy" onerror="this.style.display=\'none\'">':'<div class="vaga-logo vaga-logo-fallback">'+esc(nome.charAt(0).toUpperCase())+'</div>')+'<div class="vaga-identidade-copy"><h3>'+esc(tituloVaga(v))+'</h3><p>'+esc(nome)+seloEmpresa+'</p></div></div><div class="vaga-meta"><span><i class="meta-ico">⌖</i>'+esc(v.cidade||'')+(v.estado?' - '+esc(v.estado):'')+'</span><span><i class="meta-ico">▣</i>'+esc(v.modalidade||'')+'</span><span><i class="meta-ico">▤</i>'+esc(v.contrato||'')+'</span></div>'+(desc?'<div class="vaga-resumo-area"><small>SOBRE A VAGA</small><p class="vaga-resumo">'+esc(desc.slice(0,120))+(desc.length>120?'…':'')+'</p></div>':'<div class="vaga-resumo-area"><small>SOBRE A VAGA</small><p class="vaga-resumo">Confira os detalhes completos desta oportunidade.</p></div>')+'<div class="vaga-card-rodape"><div class="vaga-rodape-salario"><small>Salário</small><strong class="salario-card">'+esc(sal)+'</strong></div><small class="data-card-em">Publicada '+esc(textoDataVaga(v).replace(/^Publicada\s*/i,''))+'</small><button type="button" class="vaga-ver-btn" onclick="event.stopPropagation();abrirVaga(\''+v.id+'\')">Ver vaga <b>→</b></button></div></article>';
}
