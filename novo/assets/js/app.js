...
function cardVagaPortal(v){
 ...
 return '<article class="vaga-card portal-vaga portal-vaga-nova '+(destaqueAtivo(v)?'destaque':'')+'" onclick="abrirVaga(\\''+v.id+'\\')">'
 +'<div class="vaga-card-topo"><div class="vaga-selos">'+tags+'</div><button type="button" class="vaga-favorito" aria-label="Salvar vaga" onclick="event.stopPropagation();alternarSalvarVagaCard(\\''+v.id+'\\')">'+(vagaEstaSalva(v.id)?'♥':'♡')+'</button></div>'
 +'<div class="vaga-main"><div class="vaga-brand">'+(logo?'<img class="vaga-logo" src="'+esc(logo)+'" alt="Logo '+esc(nome)+'" loading="lazy" onerror="this.style.display=\\'none\\'">':'<div class="vaga-logo vaga-logo-fallback">'+esc(nome.charAt(0).toUpperCase())+'</div>')+'</div><div class="vaga-copy"><h3>'+esc(tituloVaga(v))+'</h3><p class="vaga-empresa">'+esc(nome)+seloEmpresa+'</p><p class="vaga-resumo-card">'+esc(desc||'Confira os detalhes desta oportunidade e os requisitos para participar do processo seletivo.')+'</p></div></div>'
 +'<div class="vaga-meta"><span><i class="meta-svg"><svg viewBox="0 0 24 24"><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="2.5"/></svg></i>'+esc(v.cidade||'')+(v.estado?' - '+esc(v.estado):'')+'</span><span><i class="meta-svg"><svg viewBox="0 0 24 24"><rect x="3" y="7" width="18" height="13" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></i>'+esc(v.modalidade||'')+'</span><span><i class="meta-svg"><svg viewBox="0 0 24 24"><path d="M6 2h9l4 4v16H6z"/><path d="M14 2v5h5M9 12h7M9 16h7"/></svg></i>'+esc(v.contrato||'')+'</span></div>'
 +'<div class="vaga-publicacao">Publicada '+esc(textoDataVaga(v).replace(/^Publicada\\s*/i,''))+'</div>'
 +'<div class="vaga-card-rodape"><div class="vaga-rodape-salario"><small>SALÁRIO</small><strong class="salario-card">'+esc(sal)+'</strong></div><button type="button" class="vaga-ver-btn" onclick="event.stopPropagation();abrirVaga(\\''+v.id+'\\')">Ver vaga <b>→</b></button></div>'
 +'</article>';
}
