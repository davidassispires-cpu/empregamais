...
function cardVagaPortal(v){
 ...
 const logo=!v.confidencial?(v.logo||v.logoUrl||v.empresaLogo||empLogoRecente.logo||empLogoRecente.logoUrl||empLogoRecente.perfil?.logo||''):'';
...
function cardVagaDestaqueEM(v){
 ...
 return '<article class="vaga-card portal-vaga portal-vaga-nova '+(destaqueAtivo(v)?'destaque ':'')+(v.urgente?'urgente':'')+'" onclick="abrirVaga(\''+v.id+'\')"><div class="vaga-card-topo"><div class="vaga-selos">'+tags+'</div></div><div class="vaga-identidade">'+logoHtml+'<div class="vaga-identidade-copy"><h3>'+esc(tituloVaga(v))+'</h3><p>'+esc(nome)+seloEmpresa+'</p></div></div><div class="vaga-meta"><span><i class="meta-ico">⌖</i>'+esc(v.cidade||'')+(v.estado?' - '+esc(v.estado):'')+'</span><span><i class="meta-ico">▣</i>'+esc(v.modalidade||'')+'</span><span><i class="meta-ico">▤</i>'+esc(v.contrato||'')+'</span></div>'+(desc?'<div class="vaga-resumo-area"><small>SOBRE A VAGA</small><p class="vaga-resumo">'+esc(desc.slice(0,120))+(desc.length>120?'…':'')+'</p></div>':'')+'<div class="vaga-card-rodape"><div class="vaga-rodape-salario"><small>Salário</small><strong class="salario-card">'+esc(sal)+'</strong></div><small class="data-card-em">Publicada '+esc(textoDataVaga(v).replace(/^Publicada\\s*/i,''))+'</small><button type="button" class="vaga-ver-btn" onclick="event.stopPropagation();abrirVaga(\''+v.id+'\')">Ver vaga <b>→</b></button></div></article>';
}
