...
function restaurarCandidaturaInternaEM(){const form=document.getElementById('formCandidatura');if(!form)return;form.classList.remove('cand-fluxo-externo');form.querySelectorAll('.cand-apply-person,.candidatura-resumo,.cand-apply-section,.cand-apply-checklist,.cand-aderencia-orientacao,.cand-apply-message,.cand-apply-privacy').forEach(x=>x.style.display='');const ext=document.getElementById('candFluxoExternoEM');if(ext)ext.remove();const submit=form.querySelector('button[type="submit"]');if(submit){submit.style.display='';submit.textContent='Enviar candidatura →'}const h=document.querySelector('#pagina-candidatar .cand-apply-head-copy span');if(h)h.textContent='ENVIAR CANDIDATURA';const p=document.querySelector('#pagina-candidatar .cand-apply-head-copy p');if(p)p.textContent='Revise as informações antes de enviar sua candidatura.'}
function renderizarCandidaturaExternaEM(v,tipo){
 const form=document.getElementById('formCandidatura');if(!form)return;restaurarCandidaturaInternaEM();form.classList.add('cand-fluxo-externo');form.querySelectorAll('.cand-apply-person,.candidatura-resumo,.cand-apply-section,.cand-apply-checklist,.cand-aderencia-orientacao,.cand-apply-message,.cand-apply-privacy').forEach(x=>x.style.display='none');const ader=document.getElementById('candAderenciaTopo');if(ader)ader.style.display='none';const perguntas=document.getElementById('candPerguntasEliminatoriasEM');if(perguntas)perguntas.hidden=true;
