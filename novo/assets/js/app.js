...
function renderizarEmpresaPublica(){
 ...
 const ab=$('#empresaPublicaAvaliacoes');if(ab){const media=avs.length?avs.reduce((s,x)=>s+Number(x.nota||0),0)/avs.length:0,recom=avs.length?Math.round(avs.filter(x=>x.recomenda==='sim').length/avs.length*100):0;ab.innerHTML=avs.length?'<div class="empresa-avaliacoes-head"><div class="empresa-avaliacoes-nota"><strong>'+media.toFixed(1)+'</strong><span>★★★★★</span><small>'+avs.length+' avaliação(ões) verificadas</small></div><div class="empresa-avaliacoes-recomenda"><strong>'+recom+'%</strong><span>recomendariam participar do processo seletivo</span></div></div><div class="empresa-avaliacoes-lista">'+avs.map(x=>'<article><header><div><b>Candidato verificado</b><span class="avaliacao-verificada">✓ Participou do processo</span></div><strong>'+Number(x.nota||0).toFixed(1)+' ★</strong></header>'+(x.comentario?'<p>'+esc(x.comentario)+'</p>':'')+'<footer><span>Comunicação '+(x.comunicacao||'-')+'/5</span><span>Clareza '+(x.clareza||'-')+'/5</span><span>Organização '+(x.organizacao||'-')+'/5</span><span>Respeito '+(x.respeito||'-')+'/5</span><span>Feedback '+(x.feedback||'-')+'/5</span></footer></article>').join('')+'</div>':empresaPublicaVazioEM('Ainda não há avaliações','Somente candidatos que participaram de processos seletivos desta empresa podem publicar uma avaliação.','★')}
...
function abrirMetricaVagaEM(tipo,id){sessionStorage.setItem('vagaCandidatosSelecionada',id);sessionStorage.setItem('filtroCandidatosEmpresaEM',tipo);irPara('candidatos-empresa');setTimeout(()=>{if(typeof aplicarFiltroMetricaCandidatosEM==='function')aplicarFiltroMetricaCandidatosEM(tipo)},120)}
...
function renderizarCandidatosEmpresa(){
 ...
 const todosBase=candidaturas().filter(c=>vagas.some(v=>v.id===c.vagaId)),todos=(vaga?todosBase.filter(c=>c.vagaId===vaga.id):todosBase);
 const grupos=['Todos','Em avaliação','Selecionados','Em contato','Entrevista','Aprovados','Contratados','Reprovados'];let filtro=sessionStorage.getItem('filtroCandidatos')||'Todos';const filtroMetrica=sessionStorage.getItem('filtroCandidatosEmpresaEM')||'';if(filtroMetrica==='selecionados')filtro='Selecionados';else if(filtroMetrica==='candidaturas')filtro='Todos';
...
function aplicarFiltroMetricaCandidatosEM(tipo){sessionStorage.setItem('filtroCandidatosEmpresaEM',tipo||'');if(tipo==='selecionados')sessionStorage.setItem('filtroCandidatos','Selecionados');else sessionStorage.setItem('filtroCandidatos','Todos');renderizarCandidatosEmpresa()}
...
function cardVagaPortal(v){
 ...
 const tags=(nova48?'<span class="vaga-selo nova-selo">NOVA</span>':'')+(destaqueAtivo(v)?'<span class="vaga-selo destaque-selo">EM DESTAQUE</span>':'')+(v.urgente?'<span class="vaga-selo urgente-selo">CONTRATAÇÃO URGENTE</span>':'')+(v.senior50?'<span class="vaga-selo senior-selo">50+</span>':'');
 ...
 return '<article class="vaga-card portal-vaga portal-vaga-nova '+(destaqueAtivo(v)?'destaque ':'')+(v.urgente?'vaga-urgente ':'')+'" onclick="abrirVaga(\''+v.id+'\')"><div class="vaga-card-topo"><div class="vaga-selos">'+tags+'</div><button type="button" class="vaga-favorito" aria-label="Salvar vaga" onclick="event.stopPropagation();alternarSalvarVagaCard(\''+v.id+'\')">'+(vagaEstaSalva(v.id)?'♥':'♡')+'</button></div><div class="vaga-identidade">'+(logo?'<img class="vaga-logo" src="'+esc(logo)+'" alt="Logo '+esc(nome)+'" loading="lazy" onerror="this.style.display=\'none\'">':'<div class="vaga-logo vaga-logo-fallback">'+esc(nome.charAt(0).toUpperCase())+'</div>')+'<div class="vaga-identidade-copy"><h3>'+esc(tituloVaga(v))+'</h3><p>'+esc(nome)+seloEmpresa+'</p></div></div><div class="vaga-meta"><span class="vaga-meta-local"><i class="meta-svg" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M20 10c0 5-8 11-8 11S4 15 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="2.5"/></svg></i>'+esc(v.cidade||'')+(v.estado?' - '+esc(v.estado):'')+'</span><span><i class="meta-svg" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="2"/><path d="M8 19v2m8-2v2M8 9h8"/></svg></i>'+esc(v.modalidade||'')+'</span><span><i class="meta-svg" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="3" y="7" width="18" height="13" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M3 12h18"/></svg></i>'+esc(v.contrato||'')+'</span></div><div class="vaga-resumo-area"><small>Sobre a vaga</small><p class="vaga-resumo">'+esc(desc||'Confira os detalhes desta oportunidade e os requisitos para participar do processo seletivo.')+'</p></div><div class="vaga-salario-data"><small class="data-card">Publicada '+esc(textoDataVaga(v).replace(/^Publicada\s*/i,''))+'</small></div><div class="vaga-card-rodape"><div class="vaga-rodape-salario"><small>Salário</small><strong class="salario-card">'+esc(sal)+'</strong></div><button type="button" class="vaga-ver-btn" onclick="event.stopPropagation();abrirVaga(\''+v.id+'\')">Ver vaga <b>→</b></button></div></article>';
}
